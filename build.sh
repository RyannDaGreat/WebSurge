...
#   `shell` is REQUIRED and deeply non-obvious. AudioWorkletGlobalScope defines
#   neither `window`, nor `WorkerGlobalScope`, nor `process`, so Emscripten's
#   environment detection falls through to "shell" and aborts. In a release
#   build that surfaces only as a bare wasm `unreachable` trap with no message.
#   Without `shell` the synth never starts in a worklet at all.
#   `node` is included so tools/verify_audio.mjs can render and inspect real
#   samples headlessly -- a compile is not evidence that the synth makes sound.
...
if [ "$MODE" = "release" ]; then
    # AudioWorkletGlobalScope cannot `import`, so the Emscripten glue has to be
    # in the same file as the processor and share its global scope. Concatenate
    # rather than bundle: the glue is already a self-contained UMD blob, and a
    # bundler would try to rewrite its module plumbing.
    BUNDLE="$REPO_ROOT/src/js/surge-worklet-bundle.js"
    log "Bundling worklet -> src/js/surge-worklet-bundle.js"
    {
        echo "/* GENERATED by build.sh -- do not edit."
        echo "   worklet-prelude.js + surge-engine.js (Emscripten glue) + surge-worklet.js,"
        echo "   concatenated because AudioWorkletGlobalScope has no module loader. */"
        # Prelude first: it defines globals (performance, location) that the glue
        # reads during startup and that a worklet scope does not provide.
        cat "$REPO_ROOT/src/js/worklet-prelude.js"
        echo
        cat "$OUT"
        echo
        # The worklet source is an ES module for editing/linting convenience;
        # strip the export/import syntax that a classic worklet script rejects.
        sed -e 's/^export //' "$REPO_ROOT/src/js/surge-worklet.js"
    } > "$BUNDLE"
